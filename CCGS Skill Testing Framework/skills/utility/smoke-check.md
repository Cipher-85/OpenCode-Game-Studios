...
against sprint stories, and uses `question` to batch-verify manual smoke
...
5. Uses `question` for Batch 1 (core stability) and Batch 2 (sprint mechanics)
...
- [ ] `question` is used for manual smoke check batches
...
### Case 3: Manual Confirmation — question used, PASS WITH WARNINGS
...
3. `question` is used for Batch 1 and Batch 2 — developer confirms all PASS
...
**Assertions:**
- [ ] `question` is used for manual smoke check batches (not inline text prompts)
...
- [ ] Uses `question` for all manual smoke check batches (Batch 1, Batch 2, Batch 3)
...
- The `--platform` argument adds platform-specific question batches and a
