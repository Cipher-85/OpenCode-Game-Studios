...
2. question: user approves Phase 1 output before Phase 2 begins
...
8. question: user approves the full season plan
...
4. question presented with resolution options before season plan approval is offered
...
- [ ] question is used to surface the violation and offer resolution options (revise economy design, override with documented rationale, cancel)
...
- [ ] If economy-designer blocks but analytics-engineer succeeds, analytics output is preserved and the block is surfaced via question
...
- [ ] `question` used at every phase transition — user approves before the next phase begins
...
- [ ] Error recovery: any BLOCKED agent is surfaced immediately with question options (skip / retry / stop)
