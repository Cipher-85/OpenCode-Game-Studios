...
Uses `question` at each phase transition. Delegates all file writes to
...
2. Phase 1b — `/ux-design inventory-screen` invoked (or ux-designer spawned directly); produces `design/ux/inventory-screen.md` using `ux-spec.md` template; `question` confirms spec before review
3. Phase 1c — `/ux-review design/ux/inventory-screen.md` invoked; returns APPROVED; gate passed, proceed to Phase 2
4. Phase 2 — art-director spawned; reviews full UX spec (not only wireframes); applies visual treatment; verifies color contrast; produces visual design spec with asset manifest; `question` confirms before Phase 3
...
4. `question` presented with the specific flagged concerns and options:
...
- [ ] `question` presents the specific flagged concerns before offering options
...
5. `question` presents all three review results before Phase 5 begins
...
- [ ] `question` shows all three review streams' results together before Phase 5 approval
...
3. `question` presented with options:
...
- [ ] `question` offers a "create pattern library first" option (referencing `/ux-design patterns`)
