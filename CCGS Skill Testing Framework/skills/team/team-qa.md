...
- [ ] Uses `question` at phase transitions to capture user approval before proceeding
...
2. Phase 2: Spawns `qa-lead` via task; produces strategy table classifying all 4 stories; no blockers flagged; presents to user; question: user selects "Looks good — proceed to test plan"
...
5. Phase 5: Spawns `qa-tester` via task for each Visual/Feel and Integration story (2–3 stories); run in parallel; presents test cases grouped by story; question per group; user approves
...
2. Phase 6: User marks Visual/Feel story as FAIL; question collects failure description: "Animation plays at 2x speed — jitter visible on every loop"
...
**Assertions:**
- [ ] FAIL result in Phase 6 triggers question to collect the failure description before the bug report is written
...
2. Cannot infer sprint; uses question: "Which sprint or feature should QA cover?" with options to type a sprint identifier or cancel
...
- [ ] When both state files are absent, skill uses question rather than guessing (variant B)
...
- [ ] `question` used at Phase 2 (strategy review), Phase 5 (test case approval per group), and Phase 6 (per-story manual QA result)
...
- [ ] Error recovery: any BLOCKED agent is surfaced immediately with question options
