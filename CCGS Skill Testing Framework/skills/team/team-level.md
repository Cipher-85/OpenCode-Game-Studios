...
document saved to `design/levels/[level-name].md`. Uses `question` at each
...
- [ ] Uses `question` at step transitions for user approval before proceeding
...
2. Step 1 — narrative-director spawned: defines narrative purpose, key characters, dialogue triggers, emotional arc; world-builder spawned: provides lore context, environmental storytelling opportunities, world rules; `question` confirms Step 1 outputs before Step 2
3. Step 2 — level-designer spawned: designs spatial layout (critical path, optional paths, secrets), pacing curve, encounters, puzzles, entry/exit points and connections to adjacent areas; `question` confirms layout before Step 3
4. Step 3 — systems-designer spawned: specifies enemy compositions, loot tables, difficulty balance, area-specific mechanics, resource distribution; `question` confirms systems before Step 4
5. Step 4 — art-director and accessibility-specialist spawned in parallel; art-director: visual theme, color palette, lighting, asset list, VFX needs; accessibility-specialist: navigation clarity, colorblind safety, cognitive load check — each concern rated BLOCKING / RECOMMENDED / NICE TO HAVE; `question` presents both outputs before Step 5
...
- [ ] `question` called at each step gate (minimum: after Step 1, Step 2, Step 3, Step 4)
...
4. `question` presented with options:
...
- [ ] `question` offers at minimum three options (skip / retry / stop)
...
- [ ] Usage message includes the argument-hint format from `metadata`
...
4. Skill presents both Step 4 results via `question` — BLOCKING concern highlighted prominently
5. `question` offers:
...
- [ ] `question` presents the specific concern text (not just "accessibility issue found")
...
- [ ] `question` offers a "design crystal caves first" option referencing `/team-level`
...
- [ ] `question` used at each step transition — user approves before pipeline advances
