...
and world-builder in parallel). Uses `question` at each phase transition to
...
- [ ] `question` is used at phase transitions before proceeding
...
2. `question` presents the narrative brief; user approves before Phase 2 begins
...
4. `question` presents world foundation and dialogue drafts; user approves before Phase 3 begins
5. Phase 3: level-designer is spawned; produces environmental storytelling layout, trigger placement, and pacing plan
6. `question` presents level narrative plan; user approves before Phase 4 begins
...
8. `question` presents review results; user approves before Phase 5 begins
...
- [ ] `question` appears after Phase 1 output and before Phase 2 launch
- [ ] world-builder and writer task calls are issued simultaneously in Phase 2 (not sequentially)
- [ ] level-designer is not launched until Phase 2 `question` is approved
...
- [ ] `question` presents at least 3 options including "stop and resolve first"
...
- [ ] No `question` is used — output is direct guidance
...
- [ ] `question` offers the option to fix now vs. flag and proceed
...
- [ ] `question` offers at least one option to resolve the missing prerequisite
...
- [ ] `question` is used after every phase output before the next phase launches
...
- The "Retry with narrower scope" and "Skip this agent" resolution paths from the Error
  Recovery Protocol are not separately tested — they follow the same `question`
