...
- [ ] Frontmatter has required top-level fields (`name`, `description`); original CCGS fields (`argument-hint`, `user-invocable`, `allowed-tools`) preserved under `metadata`
- [ ] 2+ phase headings found
- [ ] At least one verdict keyword present (`PASS`, `FAIL`, `CONCERNS`, `APPROVED`, `BLOCKED`, `COMPLETE`, `READY`)
- [ ] If `metadata.allowed-tools` includes Write/Edit: `"May I write"` language present
