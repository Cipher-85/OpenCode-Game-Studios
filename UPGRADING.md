...
## Migrating from Claude Code Game Studios → OpenCode

This is a native OpenCode port of Claude Code Game Studios (source commit
`984023d`). If you are migrating an existing game project from the Claude Code
version, here is what changed and how to adapt.

### Directory mapping

| Claude Code | OpenCode | Notes |
|---|---|---|
| `CLAUDE.md` | `AGENTS.md` | Collaboration protocol preserved; `@imports` → `opencode.json.instructions` |
| `.claude/settings.json` | `opencode.json` | Permissions + plugin ref + instructions |
| `.claude/agents/*.md` | `.opencode/agents/*.md` | Frontmatter converted; `tools` → `permission` deny-by-default |
| `.claude/skills/*/SKILL.md` | `.opencode/skills/*/SKILL.md` | Extras moved to `metadata` |
| (slash commands) | `.opencode/commands/*.md` | 73 new command wrappers |
| `.claude/hooks/*.sh` | `.opencode/hooks/*.sh` | Path-adapted; invoked by `ccgs-hooks.js` plugin |
| (new) | `.opencode/plugins/ccgs-hooks.js` | Event adapter |
| `.claude/rules/*.md` | nested `AGENTS.md` + `.opencode/rules/` | Path-scoping via nested AGENTS.md |
| `design/CLAUDE.md` etc. | `design/AGENTS.md` etc. | Mechanical rename |
| `.claude/statusline.sh` | `.opencode/hooks/statusline.sh` | On-demand via `/studio-status` |

### What to do

1. **Copy your game content** (`src/`, `assets/`, `design/`, `docs/`,
   `production/`, `tests/`, `prototypes/`) directly — these are unchanged.
2. **Re-apply your engine config** to `.opencode/docs/technical-preferences.md`
   (was `.claude/docs/technical-preferences.md`).
3. **Re-apply any custom permission rules** to `opencode.json` (was
   `.claude/settings.json`).
4. **Drop the `.claude/` directory** — the OpenCode port is `.opencode/`-only
   to avoid duplicate skill loading.
5. Run `/start` to verify onboarding, or `/project-stage-detect` if you have
   existing work.

See [PORTING_NOTES.md](PORTING_NOTES.md) for the full conversion log, locked
