...
# Only act on files inside .opencode/skills/
if ! echo "$FILE_PATH" | grep -qE '(^|/)\.opencode/skills/'; then
    exit 0
fi

# Extract skill name from path (.opencode/skills/[skill-name]/SKILL.md)
SKILL_NAME=$(echo "$FILE_PATH" | grep -oE '\.opencode/skills/[^/]+' | sed 's|\.opencode/skills/||')
